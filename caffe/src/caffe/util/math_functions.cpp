...
GEMM代表的是GEneralized Matrix Multiplication计算矩阵乘法的函数之一是 cblas_sgemm,
使用单精度实数。另外还有双精度实数，单精度复数和双精度复数。
在此以 cblas_sgemm为例,得到的结果是: C = alpha*op( A )*op( B ) + beta*C
函数定义为：
void cblas_sgemm(
const enum CBLAS_ORDER Order, ///行主序还是列主序,行主序用CblasRowMajor,列主序CblasColMajor
...
/*
函数 void *memcpy(void *dest, void *src, unsigned int count)
把src所指向的内存区域 copy到dest所指向的内存区域, count为块的大小
